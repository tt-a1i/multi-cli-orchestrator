...
`v0.1.0` establishes a usable baseline for multi-provider code review orchestration:
- Unified entrypoint: `mco review`
- 5-provider adapter wiring: `claude`, `codex`, `gemini`, `opencode`, `qwen`
- Wait-all parallel fan-out with idempotency/retry semantics
- Structured parse contract gate with canonical findings output
...
```bash
git clone git@github.com:tt-a1i/multi-cli-orchestrator.git
